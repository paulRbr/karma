...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  webServer.on('error', function(e) {
    if (e.code === 'EADDRINUSE') {
      ...
      config.port++;
      if (config.hostname !== constant.DEFAULT_HOSTNAME) {
        webServer.listen(config.port, hostname);
      } else {
        webServer.listen(config.ports);
      }
    } else {
      throw e;
    }
  ...
  if (config.hostname !== constant.DEFAULT_HOSTNAME) {
    webServer.listen(config.port, config.hostname, startCallback);
  } else {
    webServer.listen(config.port, startCallback);
  }
